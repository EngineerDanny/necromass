...
# Get scores from benchmark results
...
fwrite(score.tall, paste0( dataname, "_11_15.score.tall" ) )
...
gg <- ggplot(score_summary, aes(x = mean_log_mse, y = algorithm_label)) +
  ...
  scale_color_manual(values = c("black", "red")) +
  labs(title = dataname,
       x = "Mean log(Regression MSE) ± SE",  # Updated to show SE
       y = "Algorithm") +
  theme_bw() +
  theme(
    strip.text = element_text(size = 10, face = "bold"),
...
print(gg)
# Save the plot
ggsave(paste0(dataname, "_taxa_mse_11_15_results.png"), 
       plot = gg,
       width = 6, 
       height = 2,
       dpi = 300)
...
datasets <- list("necromass" = "necromass_11_15.score.tall.csv",
                "TwinsUK" = "TwinsUK_11_15.score.tall.csv",
                "qa10394" = "qa10394_11_15.score.tall.csv",
                "MovingPictures" = "MovingPictures_11_15.score.tall.csv",
                "HMPv13" = "HMPv13_11_15.score.tall.csv",
                "HMPv35" = "HMPv35_11_15.score.tall.csv")
...
gg <- ggplot(final_results, aes(x = mean_p_value, y =mean_diff, color = dataset_name)) +
 ...
 # breaks = seq(-5, 0, by = 1),
  ...
  scale_y_continuous(
    "Mean (MSE Difference) ± SE: fuser(all) - cv_glmnet(same)"
  ) +
  theme_bw()+
  theme(
    legend.position = "none",
...
# Save the plot
ggsave("mse_diff_vs_pvalue_for_fuserall_glmnetsame.png", gg, width = 10, height = 8, dpi = 300)
