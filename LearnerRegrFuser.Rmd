...
LearnerRegrFuser <- R6Class("LearnerRegrFuser",
  ...
  private = list(
    .train = function(task) {
     ...
     # glmnet_model <- glmnet::cv.glmnet(X_train, 
       #                                 y_train, 
    ...
    .predict = function(task) {
        ...
        if (any(is.na(y.predict))) {
          ...
          # Use the optimal lambda from cv.glmnet for prediction
          y_predict <- predict(glmnet_model, newx = X_test)
          y_predict <- as.vector(y_predict)
        }
        
        # Return the predictions as a numeric vector
        list(response = y_predict)
      }
