...
LearnerRegrFuser <- R6Class("LearnerRegrFuser",
  ...
  private = list(
    .train = function(task) {
      ...
      n.group <- as.numeric(table(task$groups$group)["3"]) # samples per group (for first group)
  ...
  .predict = function(task) {
      ...
      # Create a list of X matrices, one for each group
      X.list <- lapply(1:k, function(k.i) {
        group_rows <- which(group_ind == k.i)
        X[group_rows, , drop = FALSE]
      })
      # Predict response for each group
      y.predict <- unlist(lapply(1:k, function(k.i) {
        X.group <- X.list[[k.i]]
        X.group %*% beta[, k.i]
      }))
